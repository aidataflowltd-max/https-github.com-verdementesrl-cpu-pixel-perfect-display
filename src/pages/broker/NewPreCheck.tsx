...
const SOURCE_OPTIONS: { type: ConnectorType; label: string }[] = [
  { type: 'banking', label: 'Conti bancari (Open Banking / AISP)' },
  { type: 'tax', label: 'Dati fiscali (Agenzia Entrate, F24, fatture)' },
  { type: 'credit', label: 'Centrale Rischi / Credit Bureau' },
  { type: 'corporate', label: 'Camera di Commercio / Bilanci' },
  { type: 'document', label: 'Documenti caricati dall’utente' },
]

export default function NewPreCheck() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (requestError || !request) {
      setError(requestError?.message ?? 'Errore nella creazione della pratica')
      setSubmitting(false)
      return
    }
    ...
    await logAudit({
      ...
      metadata: { legalName, vat, sources: selected, origin: 'broker' },
    })
      ...
      <PortalLayout nav={NAV} title="Broker Portal">
        <div className="px-8 py-8 max-w-xl">
          <div className="card p-8 text-center">
            ...
            <h1 className="text-lg font-semibold mb-1">Pre-verifica avviata</h1>
            <p className="text-sm text-black/50 mb-6">
              Invia questo link al cliente per completare l’autenticazione e il consenso. Solo dopo aver visto
              l’esito potrai scegliere se e a quale banca inoltrare la pratica.
            </p>
            ...
            <div className="flex gap-3 justify-center flex-wrap">
              ...
              <button className="btn-primary" onClick={() => navigate('/broker/dashboard')}>Vai alla dashboard</button>
            </div>
          </div>
        </div>
      </PortalLayout>
  ...
  return (
    <PortalLayout nav={NAV} title="Broker Portal">
      <div className="px-8 py-8 max-w-2xl">
        <h1 className="text-xl font-semibold mb-1">Nuova pre-verifica</h1>
        <p className="text-sm text-black/50 mb-6">
          Verifica il cliente prima di sapere a quale banca lo indirizzerai: eviti di inviare pratiche destinate a un rifiuto.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="label">Motivo / giustificativo</label>
                <input className="input" value={financingPurpose} onChange={(e) => setFinancingPurpose(e.target.value)} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
          ...
          <div className="card p-6">
            <div className="label mb-3">Fonti da verificare</div>
            <div className="space-y-2">
              {SOURCE_OPTIONS.map((opt) => (
                <label key={opt.type} className="flex items-center gap-3 p-3 rounded-lg border border-black/[0.06] hover:bg-black/[0.015] cursor-pointer">
                  <input type="checkbox" checked={selected.includes(opt.type)} onChange={() => toggle(opt.type)} />
          ...
          <button className="btn-verified" disabled={submitting}>
            {submitting ? 'Creazione in corso…' : 'Avvia pre-verifica'}
          </button>
        </form>
      </div>
    </PortalLayout>
